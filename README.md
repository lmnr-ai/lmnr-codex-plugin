...
| `LMNR_PARENT_SPAN_CONTEXT` | — | Optional serialized Laminar span context (from `Laminar.serialize_span_context()` / SDK equivalent). When set, the Codex turn root spans are emitted as children of that span so they join the caller's trace. |
